...
function buildThreadParams(cwd, options = {}) {
  ...
  // Per-thread config override map (e.g. { model_provider, model }). Accepted by
  // codex app-server thread/start; the provider must be defined in the loaded
  // user-level config. Unofficial API — verified against codex-cli 0.142.x.
  if (options.configOverride && typeof options.configOverride === "object") {
    params.config = options.configOverride;
  }
  return params;
}
...
export async function runAppServerTurn(cwd, options = {}) {
  ...
  return runner(cwd, async (client) => {
    let threadId;
